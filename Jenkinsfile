@Library("pipeline-general@5.0.17") _ // should be updated version
pipeline {
    agent any
    parameters {
        string(name: 'BASE_URL', defaultValue: 'https://dq-qa.dq.cp.collibra.dev', description: 'Base URL for tests')
        string(name: 'DGC_BASE_URL', defaultValue: 'https://dg-qa-prevnew.collibra.com', description: 'DGC base URL for tests')
        string(name: 'ISS', defaultValue: 'automation', description: 'ISS parameter for tests')
        string(name: 'MARKS', defaultValue: 'smoke', description: 'Comma-separated test marks to include')
        string(name: 'EXCLUDE_KEYWORD', defaultValue: '', description: 'Keyword to exclude tests')
    }
    stages {
        stage('Check Branch and Run Tests') {
            steps {
                script {
                    ...
                    if (currentBranch == 'main') {
                        ...
                        echo "Skipping pipeline execution because changes are in branch: ${currentBranch}"
                        currentBuild.result = 'SUCCESS' // Mark the build as successful without running tests
                        return // Exit the pipeline without running tests
                    }
                }
            }
        }
