...
from data_test.analyze_estimate_filtergram_file_types.data_aef_sybase_up import (
    QUERY,
    CONN_NAME,
    ROW_COUNT,
    PL_JOB_EST,
...
class TestAnalyzeEstimateFiltergramSybaseUp:
    ...
    def test_analyze_estimate_filtergram_approved_sybase_up(self, api_utils):
        """Test analyze & estimate approved sybase up"""
        ...
        job_result = helper.setup_dataset(api_utils, dataset_defs, PROD_RUN_ID)
        assert_that(job_result["status"], "Job was not finished").is_equal_to("FINISHED")
        validate_job_timezone(api_utils, DATASET_NAME, PROD_RUN_ID)
