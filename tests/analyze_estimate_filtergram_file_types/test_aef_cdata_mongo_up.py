...
from data_test.analyze_estimate_filtergram_file_types.data_aef_cdata_mongo_up import (
    TRADE_DATE,
    DATASET,
    QUERY,
    CONN_NAME,
...
class TestAnalyzeEstimateFiltergramCdataMongoUp:
    ...
    def test_analyze_estimate_filtergram_approved_cdata_mongo_up(self, api_utils):
        """Test analyze, estimate, filtergram approved cdata mongo up."""
        ...
        job_result = helper.setup_dataset(api_utils, dataset_defs, TRADE_DATE)
        assert_that(job_result["status"], "Job was not finished").is_equal_to("FINISHED")
        validate_job_timezone(api_utils, DATASET, TRADE_DATE)
