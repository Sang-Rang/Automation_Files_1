...
@pytest.mark.dq_dgc
class TestDqDgcIntegrationManualRun:
    ...
    def test_dq_dgc_enable_disable_reset_integration_with_bu(self, api_utils, dgc_api_utils):
        """Verify Enable, Reset and Disable integration with Business unit and without business
        unit for Manual Run job, ref: DEV-83978"""
        dataset_run_id = DS_DEF_DGC_MANUAL_JOB_RUN_2["runId"]

        # Run a job
        job_result = helper.setup_dataset(api_utils, DS_DEF_DGC_MANUAL_JOB_RUN_2)
        ...
        dq_dgc_helper.add_dataset_to_business_unit(
            api_utils, DQ_BUSINESS_UNIT, DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME
        )

        # Run dgc integration
        dq_dgc_helper.run_dgc_integration(api_utils, DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME)
        dq_dgc_helper.submit_dgc_job_and_check_status(
            api_utils, DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME, dataset_run_id
        )

        # Verify import of DQ Job
        asset_id = dq_dgc_helper.verify_dq_job_import(
            DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME, dgc_api_utils
        )
        ...
        dq_dgc_helper.verify_imported_rule_domain(
            dgc_api_utils, DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME, DQ_BUSINESS_UNIT
        )
        ...
        # Check the breadcrumbs on DQ before. TODO: Check from here and below
        dq_breadcrumbs = api_utils.get(
            DQ_DATASET_INTEGRATION_STATUS.format(dataset=DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME)
        )
        # Validate results
        ...
        api_utils.post(DQ_RESET_INTEGRATION.format(dataset=DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME))
        ...
        integration_response = api_utils.put(
            PUT_DGC_INTEGRATIONS_STATUS.format(dataset=DS_DEF_DGC_INTEGRATION_WITH_BU_JOB_NAME),
        ...
        job_response = helper.setup_dataset(api_utils, DS_DEF_DGC_MANUAL_JOB_RUN_2)
