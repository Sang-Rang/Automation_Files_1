...
from payloads.dq_dgc_integration.pl_dq_dgc_saphana_connection import (
    DS_DEF_DGC_SAPHANA_PU_INTEGRATION,
    DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME,
    SAPHANA_SCHEMA_NAME,
    SAPHANA_TABLE_NAME,
    DQ_SAPHANA_PU_CONNECTION_NAME,
    DGC_DATABASE_UUID_SAPHANA_PU,
)
...
@pytest.mark.dq_dgc
class TestDqDgcSaphanaPuConnection:
    ...
    def test_dq_dgc_saphana_pu_connection_integration(self, api_utils, dgc_api_utils):
        """INTEGRATION - Verify SAPHANA connection and verify job can be successfully be enabled,
        ref: DEV-88275"""
        dataset_run_id = DS_DEF_DGC_SAPHANA_PU_INTEGRATION["runId"]

        # Run a job
        job_result = helper.setup_dataset(api_utils, DS_DEF_DGC_SAPHANA_PU_INTEGRATION)
        ...
        dq_dgc_helper.add_dataset_to_business_unit(
            api_utils, DQ_BUSINESS_UNIT, DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME
        )

        # Run dgc integration
        dq_dgc_helper.run_dgc_integration(api_utils, DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME)
        dq_dgc_helper.submit_dgc_job_and_check_status(
            api_utils, DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME, dataset_run_id
        )

        # Verify import of DQ Job
        asset_id = dq_dgc_helper.verify_dq_job_import(
            DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME, dgc_api_utils
        )
        ...
        dq_dgc_helper.verify_imported_rule_domain(
            dgc_api_utils, DS_DEF_DGC_SAPHANA_PU_INTEGRATION_NAME, DQ_BUSINESS_UNIT
        )
