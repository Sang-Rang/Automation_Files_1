...
from payloads.dq_dgc_integration.pl_dq_dgc_snowflake_connection import (
    ...
    DQ_SNOWFLAKE_PD_CONNECTION_NAME,
    DGC_DATABASE_UUID_SNOWFLAKE_PD,
)
...
@pytest.mark.dq_dgc
class TestDqDgcSnowflakePdConnection:
    ...
    def test_dq_dgc_snowflake_pd_connection_integration(self, api_utils, dgc_api_utils):
        """INTEGRATION - Verify SNOWFLAKE pushdown connection and verify job can
        be successfully be enabled, ref: DEV-88276"""
        dataset_run_id = DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION["runId"]

        # Run a job
        job_result = helper.run_pushdown_job(api_utils, DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION)
        ...
        dq_dgc_helper.add_dataset_to_business_unit(
            api_utils, DQ_BUSINESS_UNIT, DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION_NAME
        )

        # Run dgc integration
        dq_dgc_helper.run_dgc_integration(api_utils, DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION_NAME)
        dq_dgc_helper.submit_dgc_job_and_check_status(
            api_utils, DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION_NAME, dataset_run_id
        )

        # Verify import of DQ Job
        asset_id = dq_dgc_helper.verify_dq_job_import(
            DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION_NAME, dgc_api_utils
        )
        ...
        dq_dgc_helper.verify_imported_rule_domain(
            dgc_api_utils, DS_DEF_DGC_SNOWFLAKE_PD_INTEGRATION_NAME, DQ_BUSINESS_UNIT
        )
