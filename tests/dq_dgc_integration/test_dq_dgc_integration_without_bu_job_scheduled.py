...
from payloads.dq_dgc_integration.pl_dq_dgc_shcheduled_run import (
    DS_DEF_DGC_SCHEDULED_JOB_RUN_NO_BU,
...
SCHEDULED_JOB = DS_DEF_DGC_INTEGRATION_NO_BU_SCHEDULED_JOB_NAME
...
@pytest.mark.dq_dgc
class TestDqDgcNoBuScheduledRun:
    ...
    def test_dq_dgc_enable_disable_reset_integration_without_bu(self, api_utils, dgc_api_utils):
        ...
        dataset_run_id = DS_DEF_DGC_SCHEDULED_JOB_RUN_NO_BU["runId"]

        # Run a job
        job_result = helper.setup_dataset(api_utils, DS_DEF_DGC_SCHEDULED_JOB_RUN_NO_BU)
        assert_that(job_result["status"], "Job was not finished").is_equal_to("FINISHED")

        # Run dgc integration
        dq_dgc_helper.run_dgc_integration(
            api_utils, DS_DEF_DGC_INTEGRATION_NO_BU_SCHEDULED_JOB_NAME
        )
        dq_dgc_helper.submit_dgc_job_and_check_status(
            api_utils, DS_DEF_DGC_INTEGRATION_NO_BU_SCHEDULED_JOB_NAME, dataset_run_id
        )

        # Verify import of DQ Job
        asset_id = dq_dgc_helper.verify_dq_job_import(
            DS_DEF_DGC_INTEGRATION_NO_BU_SCHEDULED_JOB_NAME, dgc_api_utils
        )
        ...
        api_utils.post(
            DQ_RESET_INTEGRATION.format(dataset=DS_DEF_DGC_INTEGRATION_NO_BU_SCHEDULED_JOB_NAME)
        )
        ...
        scheduled_data = SCHEDULE_DATA_PAYLOAD_NO_BU
